...
export class PlacesSearcher {
  private mapsTools: MapsProvider;

  constructor() {
    // Check MAP_API_PROVIDER environment variable, default to "google"
    const provider = process.env.MAP_API_PROVIDER?.toLowerCase() || "google";
    
    if (provider === "osm" || provider === "openstreetmap") {
      this.mapsTools = new OpenStreetMapTools();
    } else if (provider === "riskscape") {
      this.mapsTools = new RiskScapeTools();
    } else {
      // Default to Google Maps for backward compatibility
  ...
  async getPlaceDetails(placeId: string): Promise<PlaceDetailsResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Error occurred while getting details",
      };
    }
  }
  ...
  async geocode(address: string): Promise<GeocodeResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Error occurred during geocoding",
      };
    }
  }
  ...
  async reverseGeocode(latitude: number, longitude: number): Promise<ReverseGeocodeResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Error occurred during reverse geocoding",
      };
    }
  }
  ...
  async getDirections(origin: string, destination: string, mode: "driving" | "walking" | "bicycling" | "transit" = "driving", departure_time?: string, arrival_time?: string): Promise<DirectionsResponse> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Error occurred while getting directions",
      };
    }
  }
